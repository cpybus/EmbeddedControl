...
void main(void)
{
	...
	while(1)
	{
		...
		printf("%5d-gx  %5d-gy  %5d-fgx  %5d-fgy  %4d-MotoPW  %4d-SterPW\n\r", gx, gy, (gx-x0), (gy-y0), MOTOR_PW, STEERING_PW);
	}	
	...
	ks = getDesiredHeading();

	//Finish these
	getDesiredGainRanger();
	getDesiredGainCompass();


	lcd_clear();
	lcd_print("Values:             ");
	lcd_print("Heading: %u", desiredHeading);
	lcd_print("\nGain Ranger: %u", gain_ranger);
	lcd_print("\nGain Compass: %f", gain_compass);
	...
	//Begin infinite loop for something??
	while(1)
	{
...
void SteeringServo()
{
	...
	if(STEERING_PW > 3380) // check if less than pulsewidth minimum
	{
		STEERING_PW = 3380;    // set SERVO_PW to a minimum value
	}
...
{
	/*
	MOTOR_PW = 2780 + kdy * gy;
	MOTOR_PW += kdx * abs(gx);
	error_sum += gy + abs(gx);
	MOTOR_PW += kdx * abs(gx) + ki * error_sum;
	if(MOTOR_PW > PW_MAX)
		MOTOR_PW = 3380;
	...
	*/
	...
	unsigned int temp1 = abs((gy-y0) * kd_y);
	unsigned int temp2 = abs((gx-x0) * kd_x);
	
	if(temp1 > temp2)
	{
		MOTOR_PW = PW_NEUT - temp1;
	}
	else
	{
		MOTOR_PW = PW_NEUT - temp2;
	}
